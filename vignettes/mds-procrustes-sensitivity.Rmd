...
full_matrix <- simulate_genetic_distances(n_pops = 40, verbose = FALSE)$distance_matrix
...
prepare_matrices_from_matrix <- function(full_matrix, k, bias_mu = 0.1, noise_sd = 0.05, randomize_shared = TRUE, seed = 42) {
  ...
  A <- abs(A + matrix(rnorm(length(A), 0, noise_sd), nrow(A)))
  B <- abs(B + matrix(rnorm(length(B), bias_mu, noise_sd), nrow(B)))
  diag(A) <- 0; diag(B) <- 0
